...
myModule.controller('IdeaCtrl', function($scope, $log, $routeParams, $location, $localStorage, AppendixFunctionsService, restPlanningService, restUserService) {
        ...
        if($localStorage.userConnected){
            restUserService.getIsTokenValid($scope.userConnected.id).then(function (isTokenStilValid) { //217 = idUser
                if(isTokenStilValid){
                    ...
                    $log.error("[IdeaCtrl] -isTokenValidOrExpired()- USER IS STILL CONNECTED ->  do nothing")
                }else{
                    //user not connected anymore
                    $log.error("[IdeaCtrl] -isTokenValidOrExpired()- USER SHOULD NOT BE CONNECTED ANYMORE -> logout()")
